...
export default function TermsOfServicePage() {
    ...
    const fetchData = async () => {
        try {
            const response = await axiosInstance.get('/section-content/terms_of_service');
            if (response.data) {
                setFormData({
                    heading: response.data.heading || 'Terms of Service',
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await axiosInstance.post('/section-content/terms_of_service', {
                ...formData,
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Terms of Service Editor</h1>
                    <p className="mt-1 text-sm text-gray-500">Manage your terms of service content.</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={isSaving}
